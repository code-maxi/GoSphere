...
public class GoServer implements Runnable {
    ...
    public static String FORMAT_ID(int id) {
    ...
    public String joinGame(GoJoin join, GoUser user) {
        ...
        if (game == null)
            return "The game with ID [" + FORMAT_ID(join.id) + "] does not exist.";
        else {
            user.name = join.name;
            user.game = game;
    ...
    public String createNewGame(GoConfig conf, GoUser user) {
        ...
        boolean cube_notok = conf.object == GoConfig.CUBE_OBJECT && (conf.n < 1 || conf.n > 20);
        if (used_ids.contains(id))
            return "The preferred ID " + id + " is in use.";
        else if (conf.object != GoConfig.CUBE_OBJECT && conf.object != GoConfig.SPHERE_OBJECT)
            return "The object must be either <0> or <1>.";
        else if (sphere_notok)
            return "The Size must be beween 0 and 40 and it must be divisible by 4.";
        else if (cube_notok)
            return "The Size must be beween 1 and 20.";
        else if (conf.first_color > 1)
            return "The color must be either 0 (black) or 1 (white).";
        else {
    ...
    public void run() {
        System.out.println("GoServer (Version V" + GoVersion.version + ") listening on port " + port
                + (DEBUG ? " in DEBUG mode" : "") + "...");
        while (!exit) {
            try {
