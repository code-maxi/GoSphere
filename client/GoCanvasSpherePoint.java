...
public class GoCanvasSpherePoint implements GoCanvasStoneAbstract {
    ...
    private void updatePath(Path2D.Double path, GoVector[] circ, GoMatrix Trm) {
        for (int i = 0; i < circ.length; i++) {
            ...
            if (i == 0)
                path.moveTo(renpos.com[0], renpos.com[1]);
            else
                path.lineTo(renpos.com[0], renpos.com[1]);
        }
        path.closePath();
    }
    ...
    public void update(GoMatrix Trm, GoMatrix Rot) {
        ...
        stoneCircle_path = new Path2D.Double();
        updatePath(stoneCircle_path, stoneCircle, T);
        if (color != null) {
    ...
    public void paint(Graphics2D g2, int layer, GoPosAbstract hoverpos) { // layer: 0 = behind sphere, 1 = in front of
                                                                          // sphere
