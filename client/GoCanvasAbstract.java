...
public abstract class GoCanvasAbstract extends JPanel implements ActionListener, MouseMotionListener, MouseListener, MouseWheelListener, KeyListener {
    ...
    public void mouseMoved(MouseEvent e) {
        ...
        for (GoCanvasStoneAbstract point : stones.values()) {
            boolean running_ok = state.status == GoStateAbstract.RUNNING_STATUS && point.getPos().s == 0 && state.turn == state.me;
            ...
            if (point.isHovered(e.getX(), e.getY())) {
                ...
                if (running_ok || delete_ok) hover_pos_paint = hover_pos;
                break;
            }
        }
        repaint();
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (hover_pos != null) {
            ...
            if (delta.length() <= MAX_MOUSEMOVE && (e.getButton() == MouseEvent.BUTTON1 || e.getButton() == MouseEvent.BUTTON3)) {
