...
public abstract class GoCanvasAbstract extends JPanel
        implements ActionListener, MouseMotionListener, MouseListener, MouseWheelListener, KeyListener {
    public static final double SCROLL_FAC = 0.05;
    public static final double DRAG_FAC = 0.003;
    ...
    private void sendChatMessage(boolean send) {
        if (send && !chatField.getText().equals(""))
            viewer.chatMessage(chatField.getText());
        chatField.setVisible(false);
        viewer.requestFocus();
    }
    ...
    public void chatMessage(String text) {
        ...
        new Thread(new Runnable() {
            public void run() {
                try {
                    Thread.sleep(CHAT_MESSAGE_DELAY);
    ...
    protected synchronized void updateMe() {
        ...
        for (GoCanvasStoneAbstract p : stones.values()) {
            p.update(Trm, Rot);
        }
    }
    ...
    public void mouseDragged(MouseEvent e) {
        if (drag_pos != null) {
            ...
            Rot_drag = GoMatrix.rotate(
                    new GoVector(
                            (drag_pos[1][1] - drag_pos[0][1]) * GoCanvasSphere.DRAG_FAC / scale_state,
                            (drag_pos[1][0] - drag_pos[0][0]) * GoCanvasSphere.DRAG_FAC / scale_state,
                            0));
    ...
    public void keyPressed(KeyEvent var1) {
        ...
        if (var1.getKeyCode() == KeyEvent.VK_ENTER)
